...
struct Node* deletion(struct Node* node,int value){
    ...
    node->height=height(node);
    int balance =balance_factor(node);
    if(balance_factor(node)>1 && balance_factor(node->left)==0){//RO rotation
        return rightrotation(node);
    }
    if(balance_factor(node)>1 && balance_factor(node->left)==1){//R1 rotation
        return rightrotation(node);
    }
    if(balance_factor(node)>1 && balance_factor(node->left)==-1){//R-1 rotation
        node->left=leftrotation(node->left);
        return  rightrotation(node);
    }
    if(balance_factor(node)<-1 && balance_factor(node->right)==0){//LO rotation
        return leftrotation(node);
    }
    if(balance_factor(node)<-1 && balance_factor(node->right)==-1){//L1 rotation
        return leftrotation(node);
    }
    if(balance_factor(node)<-1 && balance_factor(node->right)==1){//L-1 rotation
        node->right=rightrotation(node->right);
        return  leftrotation(node);
    }
    return node;
